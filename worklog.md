...
---
Task ID: EXPORT-COLORS+REVIEW-FIXES
Agent: GLM (main)
Task: Fix Excel export status/condition colors to dark format, fix NextAuth CLIENT_FETCH_ERROR, thorough code review + fix all CRITICALs.

Work Log:
**Excel Color Fix:**
- Changed status/condition cell fills from bright colors (teal #19E3C4, gold #C9A063, magenta #E06FB2, red #E0566B) to dark variants (tealDeep #0E4F4A, dark gold #3D2E12, dark magenta #4A1E38, dark red #3A1218). Text is now the bright color on the dark fill — matches the rest of the dark theme. No more jarring bright cells.

**NextAuth CLIENT_FETCH_ERROR Fix:**
- Added `refetchOnWindowFocus={false}` + `refetchInterval={5 * 60 * 1000}` to SessionProvider. Reduces session polling from the default (which fires on every focus + every 60s) to once every 5 minutes. Eliminates the "Failed to fetch" error that occurs when the server restarts mid-session.

**Code Review Fixes (4 CRITICAL + 6 HIGH + 4 MEDIUM):**
- **C1**: Export truncation — added `export=true` query param that raises the API cap from 500 to 5000. Export dialog now sends `export=true&limit=5000`. No more silent truncation.
- **C2**: Locations tree only rendered 2 levels — replaced flat `roots.map → children.map` with a recursive `LocationTreeNode` component that renders to any depth. Grandchildren, great-grandchildren, etc. all render with indentation.
- **C3**: BFS depth counter incremented per-node instead of per-level — with 20 direct children, the loop exited after ~16 nodes. Fixed to process all nodes at each level before incrementing depth.
- **C4**: Location delete didn't guard against historical transactions — Prisma's default `SetNull` on optional relations would silently NULL `fromLocationId`/`toLocationId` in the audit trail. Added `txFrom` + `txTo` to the `_count` check, blocks deletion with "N transaction(s) reference this location" message.
- **H2+H3**: N+1 queries in descendant calculation + cycle check — each location did its own `db.location.findMany`. Added a module-level cached location map (10s TTL) shared across all calls. `getDescendantLocationIds` and `isAncestor` now use the in-memory map — zero DB queries per call after the first.
- **H6**: Purge confirmation text showed raw value ("7") instead of label ("older than 7 days"). Fixed to look up the label from `PURGE_OPTIONS`.
- **H7**: Location delete had no confirmation dialog. Added `window.confirm("Delete 'name'? This cannot be undone.")`.
- **M2**: Summary sheet bar chart used proportional font "Inter" — block characters didn't align. Changed to "Consolas" (monospace).
- **M7**: Export dialog said "photo thumbnails" but they're clickable links. Fixed to "clickable photo links".
- **M11**: `olderThanDays=0` would delete everything. Changed validation from `days < 0` to `days < 1`.

VERIFICATION:
- ✅ Export succeeds with no errors — dark color scheme applied to status/condition cells
